...
curl -s "http://${CLICKHOUSE_ADDRESS}" -u "${CLICKHOUSE_USER}:${CLICKHOUSE_PASSWORD}" --data "
    CREATE DATABASE IF NOT EXISTS ${CLICKHOUSE_DB}
"

echo "===== Creating MergeTree ====="
curl -s "http://${CLICKHOUSE_ADDRESS}?database=${CLICKHOUSE_DB}" -u "${CLICKHOUSE_USER}:${CLICKHOUSE_PASSWORD}" --data "
